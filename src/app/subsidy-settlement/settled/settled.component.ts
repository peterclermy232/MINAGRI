...
export class SettledComponent implements OnInit {
  ...
  loadSettledInvoices() {
    ...
    this.invoiceService.getSettledInvoices().subscribe({
      next: (res) => {
        this.invoices = res;
        this.applyFilters();
        this.loading = false;
        console.log('Settled invoices:', this.invoices);
      },
      error: (err) => {
        console.error('Error loading settled invoices:', err);
        this.error = 'Failed to load settled invoices';
        this.loading = false;
      }
    });
  }
  ...
  applyFilters() {
    ...
    if (this.searchTerm) {
      ...
      filtered = filtered.filter(invoice =>
        ...
        invoice.payment_reference?.toLowerCase().includes(term)
      );
    }
    ...
    if (this.dateFrom) {
      filtered = filtered.filter(invoice =>
        new Date(invoice.settlement_date) >= new Date(this.dateFrom)
      );
    }

    if (this.dateTo) {
      filtered = filtered.filter(invoice =>
        new Date(invoice.settlement_date) <= new Date(this.dateTo)
      );
    }
  ...
  exportToExcel() {
    ...
    const rows = this.filteredInvoices.map(invoice => [
      ...
      this.formatDate(invoice.settlement_date),
    ...
    link.href = url;
    link.download = `settled-invoices-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
